...
var current_preset: int:
	set(value):
		if value == -2: # view preset_match's comment
			...
			GameInfo.current_preset = current_preset
...
var sleep_assurance_points: int:
	set(value):
		sleep_assurance_points = clampi(value,1,32)
...
func _ready() -> void:
	...
	current_preset = GameInfo.current_preset
	enabled_ids = GameInfo.enabled_ids # This is here instead of the variable's initialization to pass the reference to GameInfo.enabled_ids to the preset manager's enabled_ids, as otherwise it makes a copy for some reason?
...
func _process(delta: float) -> void:
	if Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT) and hovering_sleep_assurance:
		...
		if sleep_assurance_points != max(1,1 + int(mouse_pos.x/32)) + (16 * int(mouse_pos.y > (sleep_assurance_grid.size.y/2))):
			sleep_assurance_points = max(1,1 + int(mouse_pos.x/32)) + (16 * int(mouse_pos.y > (sleep_assurance_grid.size.y/2)))
