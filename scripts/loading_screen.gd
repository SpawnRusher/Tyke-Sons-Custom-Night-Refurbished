...
func _input(event: InputEvent) -> void:
	if event is InputEventKey and event.is_pressed():
		if version_type != PastebinChecks.VERSION_TYPE.DISABLED:
...
func _pastebin_version_check(vt: PastebinChecks.VERSION_TYPE, pb_v: String) -> void:
	version_type = vt
	if version_type == PastebinChecks.VERSION_TYPE.DEV:
		...
		SceneManager.change_to_scene("res://scenes/menu.tscn",SceneManager.CHANGE_SCENE_BEHAVIOR.AWAIT)
	elif version_type == PastebinChecks.VERSION_TYPE.DISABLED:
	...
	elif version_type == PastebinChecks.VERSION_TYPE.OUTDATED:
		...
		SceneManager.change_to_scene("res://scenes/menu.tscn",SceneManager.CHANGE_SCENE_BEHAVIOR.AWAIT)
	elif version_type == PastebinChecks.VERSION_TYPE.LATEST:
